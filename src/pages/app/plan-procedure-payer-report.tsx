...
import {
  fetchInsuranceData,
  getPlanProcedurePayerHistoryData,
...
import type {
  ...
  PlanProcedurePayerReportsResults,
  PlanProcedurePayerReportsSummary,
...
const PlanProcedurePayerReport = () => {
  ...
  const defaultSearchCriteria: PlanProcedurePayerHistoryCriteria = {
    groupID: null,
    insuranceID: null,
    cpt: null,
    fromDate: null,
  ...
  const setSearchCriteriaFields = (
    value: PlanProcedurePayerHistoryCriteria
  ) => {
  ...
  const getSearchData = async (obj: PlanProcedurePayerHistoryCriteria) => {
    const res = await getPlanProcedurePayerHistoryData(obj);
    if (res) {
      setSearchResult(res.procedureReportByPayerData);
  ...
  const downloadPdf = (pdfExportData: PlanProcedurePayerHistoryReport) => {
    if (!pdfExportData) {
      setStatusModalInfo({
        ...statusModalInfo,
        show: true,
    ...
    if (searchCriteria.dateType) {
      criteriaArray.push({
        Criteria: 'Service Based On',
    ...
    const procedureDetails: PDFRowInput[] =
      pdfExportData.procedureReportByPayerData.map((n) => {
        return {
          Payer: n.insurance,
          Group: n.group,
          'Group EIN': n.groupEIN,
          'CPT Code': n.cptCode,
          Description: n.cptDescription,
          'CPT Count': n.cptCount.toString(),
          Modifier: n.modifier,
          'PoS Code': n.posCode,
          'Patient Count': n.patientCount.toString(),
          'Claim Count': n.claimCount.toString(),
          Allowable: currencyFormatter.format(n.totalAllowed),
          Amount: currencyFormatter.format(n.totalAmount),
          Payment: currencyFormatter.format(n.paid),
          'Write Off': currencyFormatter.format(n.adjustments),
        };
      });
    ...
    const summaryExportData: PDFRowInput[] = summaryData.map((n) => {
      return {
        'Total CPT Count': n.totalCount,
        'Total Patient Count': n.totalPatientCount,
        'Total Claim Count': n.totalClaimCount,
        'Total Allowed': currencyFormatter.format(n.totalAllowed),
        'Total Amount': currencyFormatter.format(n.totalAmount),
        'Total Payment': currencyFormatter.format(n.totalPayment),
        'Total Write-Off': currencyFormatter.format(n.totalWriteOff),
      };
    });
    ...
    ExportDataToPDF(data, 'Plan Procedure Payer History');
    dispatch(
  ...
  const ExportData = async (type: string) => {
    const obj: PlanProcedurePayerHistoryCriteria = {
    ...
    if (res) {
      if (type === 'pdf') {
        ...
        const exportDataArray = res.procedureReportByPayerData.map((n) => {
          return {
            Payer: n.insurance,
            Group: n.group,
            'Group EIN': n.groupEIN,
        ...
        if (exportDataArray.length !== 0) {
          ...
          criteriaObj = {
            ...criteriaObj,
            Payer: 'CPT Code',
            Group: searchCriteria.cpt || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Payer: 'Date - From',
            Group: searchCriteria.fromDate || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Payer: 'Date - To',
            Group: searchCriteria.toDate || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Payer: 'Service Based On',
            Group: searchCriteria.dateType || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          ...
          criteriaObj = {
            ...criteriaObj,
            Payer: 'Total Patient Count',
            Group:
              (res.summary[0] && res.summary[0].totalPatientCount.toString()) ||
              '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Payer: 'Total Claim Count',
            Group:
              (res.summary[0] && res.summary[0].totalClaimCount.toString()) ||
              '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...
            Group:
              (res.summary[0] &&
                currencyFormatter
                  .format(res.summary[0].totalAllowed)
                  .toString()) ||
              '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          ...
          criteriaObj = {
            ...criteriaObj,
            Payer: 'Plan Procedure Payer History Details',
            Group: '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = Object.fromEntries(headerArray.map((key) => [key, '']));
          ...
          if (type === 'csv') {
            ExportDataToCSV(exportArray, 'PlanProcedurePayerHistory');
            dispatch(
              addToastNotification({
                text: 'Export Successful',
                toastType: ToastType.SUCCESS,
  ...
  return (
    <AppLayout title="Nucleus - Plan Procedure Payer">
                    ...
                    <div className="flex items-start justify-between gap-4  px-7 pt-[33px] pb-[21px]">
                      <div className="flex h-[38px] gap-6">
                        <p className="self-center text-3xl font-bold text-cyan-700">
                          Plan Procedure History Report by Payer
                        </p>
                      </div>
                        ...
                        <div className="w-full">
                          ...
                          <div className="w-[50%] lg:w-[70%]">
                            ...
                            <div className="flex w-full flex-wrap pl-[5px]">
                              ...
                              <div className={`w-[50%] px-[20px] lg:w-[42%]`}>
                                <div
                                  className={`w-full items-start self-stretch`}
                                >
                                  <div className="truncate py-[2px] text-sm font-medium leading-tight text-gray-700">
                    ...
                    <div hidden={reportCollapseInfo.summary} className="w-full">
                      <AppTable
                        ...
                        renderBody={
                          <>
                            <AppTableRow>
                              <AppTableCell>Total CPT Count</AppTableCell>
                              <AppTableCell>
                                {summaryData[0]?.totalCount}
                              </AppTableCell>
                            </AppTableRow>
                            <AppTableRow>
                              <AppTableCell>Total Patient Count</AppTableCell>
                              <AppTableCell>
                                {summaryData[0]?.totalPatientCount}
                              </AppTableCell>
                            </AppTableRow>
                            <AppTableRow>
                              <AppTableCell>Total Claim Count</AppTableCell>
                              <AppTableCell>
                                {summaryData[0]?.totalClaimCount}
                              </AppTableCell>
                            </AppTableRow>
                            <AppTableRow>
                              <AppTableCell>Total Allowed</AppTableCell>
                              <AppTableCell>
                                {(summaryData[0]?.totalAllowed ||
                                  summaryData[0]?.totalAllowed === 0) &&
...
export default PlanProcedurePayerReport;
