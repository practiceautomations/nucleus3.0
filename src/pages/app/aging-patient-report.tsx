...
import {
  getAgingPatienteReportDetailsData,
  getAgingPatientReportData,
...
import type {
  ...
  AgingPatientReportDetails,
  AgingPatientReportSummary,
  GroupData,
...
const AgingPatientReport = () => {
  ...
  const defaultSearchCriteria: AgingByPatientReportCriteria = {
  ...
  const setSearchCriteriaFields = (value: AgingByPatientReportCriteria) => {
  ...
  const [isAgingPatientDetailModalOpen, setIsAgingPatientDetailModalOpen] =
    useState(false);

  const [agingPatientDetailsTotalCount, setAgingPatientDetailsTotalCount] =
    useState<number>(0);
  const [agingPatientDetailsCriteria, setAgingPatientDetailsCriteria] =
    useState<AgingByPatientReportDetailsCriteria>({
      groupID: null,
      patientID: null,
      agingType: '',
      currentIndex: '',
  ...
  const [agingPatientDetailsData, setAgingPatientDetailsData] = useState<
    AgingByPatientReportDetailsResult[]
  >([]);
  const getAgingPatientDetailsData = async (
    obj: AgingByPatientReportDetailsCriteria
  ) => {
    const res = await getAgingPatienteReportDetailsData(obj);
    if (res) {
      setAgingPatientDetailsData(res);
      setAgingPatientDetailsCriteria(obj);
      setAgingPatientDetailsTotalCount(res[0]?.total || 0);
    } else {
      setStatusModalInfo({
        ...statusModalInfo,
        show: true,
  ...
  const [searchResult, setSearchResult] = useState<AgingPatientReportDetails[]>(
    []
  );
  const [summaryData, setSummaryData] = useState<AgingPatientReportSummary[]>(
    []
  );
  const getSearchData = async (obj: AgingByPatientReportCriteria) => {
    const res = await getAgingPatientReportData(obj);
    if (res) {
      setSearchResult(res.agingByPatientReportData);
      setSummaryData(res.summary);
      setTotalCount(res.agingByPatientReportData[0]?.total || 0);
      ...
      renderCell: (params) => {
        return (
          <div
            className="cursor-pointer text-sm font-normal leading-5 text-cyan-500 underline"
            onClick={() => {
              setIsAgingPatientDetailModalOpen(true);
              const obj = {
                ...agingPatientDetailsCriteria,
                groupID: searchCriteria.groupID,
                agingType: searchCriteria.agingType,
                patientID: params.row.id,
                currentIndex: 'Current',
              };
              getAgingPatientDetailsData(obj);
              setAgingPatientDetailsCriteria(obj);
            }}
          >
  ...
  const downloadPdf = (pdfExportData: AgingPatientReportData) => {
    if (!pdfExportData) {
      setStatusModalInfo({
        ...statusModalInfo,
        show: true,
    ...
    const insuranceProfileDetails: PDFRowInput[] =
      pdfExportData.agingByPatientReportData.map((m) => {
        return {
          'Patient ID': m.id,
          ...
          'Advance Balance': currencyFormatter.format(m.advanceBalance),
        };
      });
    ...
    ExportDataToPDF(data, 'Aging by Patient Report');
    dispatch(
      addToastNotification({
  ...
  const ExportData = async (type: string) => {
    const obj: AgingByPatientReportCriteria = {
      groupID: searchCriteria.groupID,
    ...
    if (res) {
      if (type === 'pdf') {
        ...
        const exportDataArray = res.agingByPatientReportData.map((n) => {
          return {
            ...
            'Advance Balance': currencyFormatter.format(n.advanceBalance),
          };
        });
        if (exportDataArray.length !== 0) {
          ...
          criteriaObj = {
            ...
            'Patient Name': searchCriteria.patientFirstName || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Patient Last Name',
            'Patient Name': searchCriteria.patientLastName || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
          ...
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Balance >',
            'Patient Name': searchCriteria.fromBalance?.toString() || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Balance <',
            'Patient Name': searchCriteria.toBalance?.toString() || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Date - From',
            'Patient Name': searchCriteria.fromDate || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Date - To',
            'Patient Name': searchCriteria.toDate || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Aging Based On',
            'Patient Name':
              searchCriteria.agingType === 'dos'
                ? 'Date of Service'
                : 'Creation Date',
          ...
          criteriaObj = {
            ...criteriaObj,
            'Patient ID': 'Aging by Patient Report',
            'Patient Name': '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = Object.fromEntries(headerArray.map((key) => [key, '']));
          ...
          if (type === 'csv') {
            ExportDataToCSV(exportArray, 'AgingByPatientReport');
            dispatch(
              addToastNotification({
                text: 'Export Successful',
                toastType: ToastType.SUCCESS,
            ...
            ExportDataToDrive(exportArray, 'AgingByPatientReport', dispatch);
          }
        }
      }
    } else {
  ...
  return (
    <AppLayout title="Nucleus - Aging Patient Report">
        ...
        <div className="m-0 h-full w-full overflow-y-scroll bg-gray-100 p-0">
          <div className="flex w-full flex-col">
            <div className="m-0 h-full w-full overflow-y-auto overflow-x-hidden bg-gray-100 p-0">
              ...
              <Modal
                open={isAgingPatientDetailModalOpen}
                onClose={() => {
                  setAgingPatientDetailsData([]);
                  setAgingPatientDetailsTotalCount(0);
                  setIsAgingPatientDetailModalOpen(false);
                }}
                  ...
                  <div className="w-full flex-1 overflow-y-auto bg-gray-100">
                    <div className="flex">
                      <div className="w-full self-center break-words p-6 text-justify text-sm text-gray-500">
                        <SearchDetailGrid
                          showTableHeading={false}
                          totalCount={agingPatientDetailsTotalCount}
                          rows={
                            agingPatientDetailsData?.map((row) => {
...
export default AgingPatientReport;
