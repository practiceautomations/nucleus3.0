...
const MonthlySummaryArReport = () => {
  ...
  const [avgRowResult, setAvgRowResult] = useState<MonthlySummaryReportData>();
  const [totalRowResult, setTotalRowResult] =
    useState<MonthlySummaryReportData>();
  ...
  const downloadPdf = (pdfExportData: MonthlySummaryReportData[]) => {
    if (!pdfExportData) {
      setStatusModalInfo({
        ...statusModalInfo,
        show: true,
    ...
    if (searchCriteria.fromDate) {
      const dateObject = StringToDatePipe(searchCriteria.fromDate);
      criteriaArray.push({
        Criteria: 'From Date',
        Value: DateToStringPipe(dateObject, 2),
      });
    }
    if (searchCriteria.toDate) {
      const dateObject = StringToDatePipe(searchCriteria.toDate);
      criteriaArray.push({
        Criteria: 'To Date',
        Value: DateToStringPipe(dateObject, 2),
      });
    ...
    const insuranceProfileDetails: PDFRowInput[] = pdfExportData.map((m) => {
      return {
        ...
        'Patient Discount': currencyFormatter.format(m.patientDiscount || 0),
  ...
  const ExportData = async (type: string) => {
    const obj: MonthlySummaryReportCriteria = {
    ...
    if (res) {
      if (type === 'pdf') {
        ...
        if (exportDataArray.length !== 0) {
          ...
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          if (searchCriteria.fromDate) {
            const dateObject1 = StringToDatePipe(searchCriteria.fromDate);
            criteriaObj = {
              ...criteriaObj,
              Month: 'Date - From',
              Charges: DateToStringPipe(dateObject1, 2) || '',
            };
          }
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));

          if (searchCriteria.toDate) {
            const dateObject2 = StringToDatePipe(searchCriteria.toDate);
            criteriaObj = {
              ...criteriaObj,
              Month: 'Date - To',
              Charges: DateToStringPipe(dateObject2, 2) || '',
            };
          }
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = Object.fromEntries(headerArray.map((key) => [key, '']));
          ...
          if (type === 'csv') {
            ExportDataToCSV(exportArray, 'MonthlySummaryReport');
            dispatch(
              addToastNotification({
                text: 'Export Successful',
                toastType: ToastType.SUCCESS,
            ...
            ExportDataToDrive(exportArray, 'MonthlySummaryReport', dispatch);
          }
        }
      }
    } else {
          ...
          <div className="flex w-full flex-col">
            <div className="m-0 h-full w-full overflow-y-auto overflow-x-hidden bg-gray-100 p-0">
              ...
              <div className="flex w-full flex-col gap-4 bg-white px-7 pt-[25px] pb-[15px]">
                ...
                <div className="no-scrollbar flex flex-col gap-3 overflow-x-auto">
                    ...
                    <>
                      <div className="inline-table w-full rounded-md border border-gray-200 bg-cyan-50 px-[24px] py-[16px] text-sm font-bold leading-5">
                        <div className="flex flex-row">
                          ...
                          <div className="w-[118px] px-2">
                            {currencyFormatter.format(
                              avgRowResult?.patientDiscount || 0
                            )}
                          </div>
                          <div className="w-[125px] px-2">
                            {currencyFormatter.format(
                      ...
                      <div className="inline-table w-full gap-2 rounded-md border border-gray-200 bg-cyan-50 px-[24px] py-[16px] text-sm font-bold leading-5">
                        <div className="flex flex-row">
                          ...
                          <div className="w-[118px] px-2">
                            {currencyFormatter.format(
                              totalRowResult?.patientDiscount || 0
                            )}
                          </div>
                          <div className="w-[125px] px-2">
                            {currencyFormatter.format(
...
export default MonthlySummaryArReport;
