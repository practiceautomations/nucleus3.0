...
import { getPatientAdvancePaymentReportData } from '@/store/shared/sagas';
import type {
  GroupData,
...
const PatientAdvancePaymentReport = () => {
  ...
  const defaultSearchCriteria: PatientAdvancePaymentReportCriteria = {
    groupID: null,
    patientFirstName: '',
    patientLastName: '',
    patientID: '',
    fromAdvanceBalance: undefined,
    toAdvanceBalance: undefined,
    fromPostingDate: '',
    toPostingDate: '',
    getAllData: false,
    getOnlyIDS: false,
  ...
  const setSearchCriteriaFields = (
    value: PatientAdvancePaymentReportCriteria
  ) => {
  ...
  const [searchResult, setSearchResult] = useState<
    PatientAdvancePaymentReportResult[]
  >([]);
  const getSearchData = async (obj: PatientAdvancePaymentReportCriteria) => {
    const res = await getPatientAdvancePaymentReportData(obj);
    if (res) {
      setSearchResult(res);
  ...
  const columns: GridColDef[] = [
      ...
      renderCell: (params) => {
        return (
          <div className="flex flex-col">
            ...
            <div>EIN: {params.row.groupEIN || ''}</div>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      headerName: 'Advance Payment Amo',
      ...usdPrice,
      flex: 1,
      minWidth: 120,
      disableReorder: true,
  ...
  const downloadPdf = (pdfExportData: PatientAdvancePaymentReportResult[]) => {
    if (!pdfExportData) {
      setStatusModalInfo({
        ...statusModalInfo,
        show: true,
    ...
    if (
      searchCriteria.fromAdvanceBalance ||
      searchCriteria.fromAdvanceBalance === 0
    ) {
      criteriaArray.push({
        Criteria: 'Balance >',
        Value: searchCriteria.fromAdvanceBalance,
      });
    }
    if (searchCriteria.toAdvanceBalance) {
      criteriaArray.push({
        Criteria: 'Balance <',
        Value: searchCriteria.toAdvanceBalance,
      });
    }
    ...
    const insuranceProfileDetails: PDFRowInput[] = pdfExportData.map((m) => {
      return {
        ...
        'Advance Payment Amo': currencyFormatter.format(m.advancePayment),
        'Applied Payment': currencyFormatter.format(m.appliedPayment),
        'Advance Balance': currencyFormatter.format(m.advanceBalance),
        'Patient Balance': currencyFormatter.format(m.patientBalance),
      };
    });
    ...
    ExportDataToPDF(data, 'Patient Advance Payment Report');
    dispatch(
  ...
  const ExportData = async (type: string) => {
    const obj: PatientAdvancePaymentReportCriteria = {
      ...
      if (type === 'pdf') {
        ...
        if (exportDataArray.length !== 0) {
          ...
          criteriaObj = {
            ...criteriaObj,
            Group: 'Balance >',
            'Patient ID': searchCriteria.fromAdvanceBalance?.toString() || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Group: 'Balance <',
            'Patient ID': searchCriteria.toAdvanceBalance?.toString() || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Group: 'Date - From',
            'Patient ID': searchCriteria.fromPostingDate || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Group: 'Date - To',
            'Patient ID': searchCriteria.toPostingDate || '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = Object.fromEntries(headerArray.map((key) => [key, '']));
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = {
            ...criteriaObj,
            Group: 'Patient Advance Payment Report',
            'Patient ID': '',
          };
          criteriaArray.push(JSON.parse(JSON.stringify(criteriaObj)));
          criteriaObj = Object.fromEntries(headerArray.map((key) => [key, '']));
          ...
          if (type === 'csv') {
            ExportDataToCSV(exportArray, 'PatientAdvancePaymentReport');
            dispatch(
              addToastNotification({
                text: 'Export Successful',
                toastType: ToastType.SUCCESS,
  ...
  return (
    <AppLayout title="Nucleus - Patient Advance Payment Report">
                    ...
                    <div className="flex items-start justify-between gap-4 bg-white px-7 pt-[33px] pb-[21px]">
                      <div className="flex h-[38px] gap-6">
                        <p className="self-center text-3xl font-bold text-cyan-700">
                          Patient Advance Payment Report
                        </p>
                      </div>
                                  ...
                                  <div className="w-full">
                                    <InputField
                                      ...
                                      onChange={(evt) => {
                                        setSearchCriteriaFields({
                                          ...searchCriteria,
                                          fromAdvanceBalance: evt.target.value
                                  ...
                                  <div className="w-full">
                                    <InputField
                                      ...
                                      onChange={(evt) => {
                                        setSearchCriteriaFields({
                                          ...searchCriteria,
                                          toAdvanceBalance: evt.target.value
                                            ? Number(evt.target.value)
                                            : undefined,
                                        });
                                      }}
...
export default PatientAdvancePaymentReport;
